...
//▮YOUTUBE CHANNELL: https://youtube.com/c/UMARMODS
//▮
//╰▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
//
//┏━━━━━━━━━━━━━━━━━━━━━━━━━
...
require('./bot')
const { default: UmarBotIncConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
say('UMAR-BOT\nV4', {
  ...
  say(`UMAR-BOT-INC By @${author.name || author}`, {
    font: 'console',
    align: 'center',
    gradient: ['red', 'green']
  })
Drips.hr();
console.log(color(''), color('THANKS FOR CHOOSING UMAR BOT', 'green'))
console.log(color(''), color('SCRIPT BY UMAR', 'red'))
console.log(color( ''), color('https://wa.me/923165123719','cyan'))
Drips.hr();
Drips.banner(`UMAR DEVIL BOT OWNER`)
Drips.ok('WELCOME TO UMARBOT')
...
async function startUmarBotInc() {
    const UmarBotInc = UmarBotIncConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['SUB DARK UMAR OFC','Ubuntu','20.0.04'],
        auth: state,
        
    })

    store.bind(UmarBotInc.ev)
    ...
    UmarBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        if (!UmarBotInc.public && !mek.key.fromMe && chatUpdate.type === 'notify') return
        if (mek.key.id.startsWith('BAE5') && mek.key.id.length === 16) return
        m = smsg(UmarBotInc, mek, store)
        require("./Unarbot")(UmarBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
 ...
 UmarBotInc.ev.on('group-participants.update', async (anu) => {
        console.log(anu)
        try {

            let metadata = await UmarBotInc.groupMetadata(anu.id)
            ...
            for (let num of participants) {
                ...
                try {
                    ppuser = await UmarBotInc.profilePictureUrl(num, 'image')
                } catch {
                ...
                try {
                    ppgroup = await UmarBotInc.profilePictureUrl(anu.id, 'image')
                } catch {
...
caption: he,
footer: `©UMAR BOT INC 2022`,
buttons: buttons,
headerType: 4,
contextInfo: { externalAdReply:{
title:"UMAR BOT V4",
body:"SUB DARK UMAR OFC",
showAdAttribution: true,
mediaType:2,
thumbnail:wangu,
mediaUrl:`https://www.youtube.com/c/UMARMODS`, 
sourceUrl: `https://www.youtube.com/c/UMARMODS`,
}}
}
const driphunny = fs.readFileSync('./Zimbot/welcome.mp3')
UmarBotInc.sendMessage(anu.id, { audio: driphunny, mimetype: 'audio/mp4', ptt: true})
UmarBotInc.sendMessage(anu.id, buttonMessage)
...
jpegThumbnail:buffer,
mentions: [num],
fileName: `Oye Rami HE/SHE IS GONE BRO ${metadata.subject}`,
fileLength: 99999999999999,
caption: he,
footer: `©UMAR BOT INC 2022`,
buttons: buttons,
...
UmarBotInc.sendMessage(anu.id, { audio: dripbabe, mimetype: 'audio/mp4', ptt: true})
UmarBotInc.sendMessage(anu.id, buttonMessage)
...
//by drips
function scheduleGc() {// garbage collector by Umar
...
//settings
    ...
    UmarBotInc.ev.on('contacts.update', update => {
    ...
    UmarBotInc.getName = (jid, withoutContact  = false) => {
        id = UmarBotInc.decodeJid(jid)
        withoutContact = UmarBotInc.withoutContact || withoutContact 
        let v
        if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
            v = store.contacts[id] || {}
            if (!(v.name || v.subject)) v = UmarBotInc.groupMetadata(id) || {}
    ...
    UmarBotInc.sendContact = async (jid, kon, quoted = '', opts = {}) => {
	let list = []
	for (let i of kon) {
	    list.push({
	...
	UmarBotInc.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
    }
    
    UmarBotInc.setStatus = (status) => {
        UmarBotInc.query({
            tag: 'iq',
            attrs: {
                to: '@s.whatsapp.net',
    ...
    UmarBotInc.public = true

    UmarBotInc.serializeM = (m) => smsg(ZimBotInc, m, store)

    UmarBotInc.ev.on('connection.update', async (update) => {
    ...
    UmarBotInc.ev.on('creds.update', saveState)
     ...
     * @param {*} options 
     */
     UmarBotInc.send5ButGif = async (jid , text = '' , footer = '', but = [], options = {}) =>{
